    ...
    public class ImplCategoriaDatos
    {
        ...
        public IEnumerable<CategoriaDbModel> ListarRegistros(String filtro, int paginaActual, int numRegistrosPorPagina, out int totalRegistros)
        {
            ...
            using (InventarioUdCEntities bd = new InventarioUdCEntities())
            {
                ...
                //lista = bd.tb_categoria.Where(x => x.nombre.Contains(filtro)).Skip(regDescartados).Take(numRegistrosPorPagina).ToList();
        ...
        public bool EliminarRegistro(int id)
        {
            try
            {
                using (InventarioUdCEntities bd = new InventarioUdCEntities())
                {
                    ...
                    //Verificación de la existencia de un registro con el mismo id
                    tb_categoria registo = bd.tb_categoria.Find(id);
                    ...
                    {
                        bd.tb_categoria.Remove(registo);
                        bd.SaveChanges();
                        return true;
                    }
                }
